		...
		for (let x = 0; x < blocks[y].length; x++) {
			for (let z = 0; z < blocks[y][x].length; z++) {
				...
				await loadModel(block, resources).then((model) => {
					...
					console.log(model);
					if (model.material) {
						model.material.emissive = new THREE.Color(0x222222); // Slight light emission
					} else if (model.children) {
						// If the model consists of multiple parts, each with its own material
						model.children.forEach((child) => {
							if (child.material) {
								child.material.emissive = new THREE.Color(0x222222); // Apply to all child materials
							}
						});
					}
					...
					if (properties) {
						// Get if upside down
						const half = properties.find((property) =>
							property.startsWith("half=")
						);
						...
						// Get facing property
						const facing = properties.find((property) =>
							property.startsWith("facing=")
						);
						if (facing) {
							// Get the direction
						...
						// Get type property
						const type = properties.find((property) =>
						...
						// Get axis property
						const axis = properties.find((property) =>
							property.startsWith("axis=")
						);
						if (axis) {
							// Get the direction
